...
public interface IQuantitiesRcdRepository extends JpaRepository<QuantitiesRcd,Long> {
    ...
    @Query(value = "DELETE FROM quantities_rcd WHERE data_generator_id = :idDataGenerator",nativeQuery = true)
    QuantitiesRcd deleteByIdDataGenerator(Long idDataGenerator);
